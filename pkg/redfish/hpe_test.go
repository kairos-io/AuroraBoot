...
func TestNewHPEClient(t *testing.T) {
...
func TestHPEClient_GetSystemInfo(t *testing.T) {
	// Create a test server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Mock authentication endpoint
		if r.URL.Path == "/redfish/v1/SessionService/Sessions" && r.Method == "POST" {
	...
	assert.Equal(t, "HPE", sysInfo.Manufacturer)
	assert.Equal(t, "ProLiant DL380 Gen10", sysInfo.Model)
	assert.Equal(t, "MXQ93701J4", sysInfo.SerialNumber)
	assert.Equal(t, 128, sysInfo.MemorySize)
	assert.Equal(t, 24, sysInfo.ProcessorCount)
}

func TestHPEClient_DeployISO(t *testing.T) {
	// Create a temporary ISO file
	tempDir, err := os.MkdirTemp("", "hpe-test")
	require.NoError(t, err)
...
func TestHPEClient_GetDeploymentStatus(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create client
			statusClient, err := NewHPEClient(statusServer.URL, "admin", "password", true, 10*time.Second)
			require.NoError(t, err)
