...
func TestNewDMTFClient(t *testing.T) {
	...
	client, err := NewVendorClient(VendorDMTF, server.URL, "admin", "password", true, 10*time.Second)
	require.NoError(t, err)
	assert.NotNil(t, client)
}

func TestDMTFClient_GetSystemInfo(t *testing.T) {
	// Create a test server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Mock authentication endpoint
		if r.URL.Path == "/redfish/v1/SessionService/Sessions" && r.Method == "POST" {
	...
	assert.Equal(t, "DMTF", sysInfo.Manufacturer)
	assert.Equal(t, "PiKVM", sysInfo.Model)
	assert.Equal(t, "0123456789", sysInfo.SerialNumber)
	assert.Equal(t, 4, sysInfo.MemorySize)
	assert.Equal(t, 4, sysInfo.ProcessorCount)
}

func TestDMTFClient_DeployISO(t *testing.T) {
	// Create a temporary ISO file
	tempDir, err := os.MkdirTemp("", "dmtf-test")
	require.NoError(t, err)
...
func TestDMTFClient_GetDeploymentStatus(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create client
			statusClient, err := NewVendorClient(VendorDMTF, statusServer.URL, "admin", "password", true, 10*time.Second)
			require.NoError(t, err)
